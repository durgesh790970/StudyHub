...
from .email_utils import send_result_email, send_welcome_email, send_login_alert_email
...
def login_page(request):
    if request.method == "POST":
        ...
        if user is not None:
            ...
            # ✅ Login alert email भेजो
            send_login_alert_email(email, user.first_name or user.username)
...
def profile(request):
    ...
    if phone:
        ...
        # Get purchased items, transactions, and test results from database
        from .models import Transaction, PDF, PurchasedItem, TestResult
        ...
        # Fetch all data for this Django user (not accounts.User)
        txs = list(Transaction.objects.filter(user=auth_user).order_by('-created_at'))
        purchased_items = list(PurchasedItem.objects.filter(user=auth_user).order_by('-purchased_at'))
        test_results = list(TestResult.objects.filter(user=auth_user).order_by('-attempt_date'))
        
        # Get purchase context for display
        purchases = []
        
        context.update({'user_obj': auth_user, 'email_user': True, 'purchased_pdfs': purchased_pdfs, 'transactions': txs, 'purchased_items': purchased_items, 'test_results': test_results, 'profile_user_id': None})
...
def pdf_download(request, pk):
    ...
    # Find current UserProfile either via phone session or linked auth_user
    acct_user = None
    phone = request.session.get('phone')
    uid = request.session.get('user_id')
    if phone:
        try:
            acct_user = UserProfile.objects.get(phone=phone)
        except UserProfile.DoesNotExist:
            acct_user = None
    elif uid:
        try:
            auth_user = User.objects.get(pk=uid)
            acct_user = UserProfile.objects.get(auth_user=auth_user)
        except Exception:
            acct_user = None
